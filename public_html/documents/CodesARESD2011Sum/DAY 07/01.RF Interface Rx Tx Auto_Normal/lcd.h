...
DESCRIPTION: 
	...
	lcd_init();------Call the lcd_init() function before calling any other 
	...
	lcd_showvalue();-This will show a 3-digit decimal value on the LCD. For example
					 if we give lcd_showvalue(0xFF) then 255 will be displayed.
	lcd_gotoxy();----Will set the cursor at a particular position on of the LCD.
					 So lcd_gotoxy(1,4) will set the cursor at the 2d row 3rd 
					 column row and column address are starts from(0,0)
...
void delay_ns(unsigned char time);
void delay_ms(unsigned int ms);
void latch(void);

void lcd_init(void)	//initialization of the lcd ****************************/1
{	
	delay_ms(20);
	save=PORTC;
	DDRC=0xFF;
...
void lcd_clear() //to reset lcd ********************************************/2
{
	lcd_cmd(0x01);
}

void lcd_gotoxy(unsigned char row,unsigned char column)	//move cursor to the specified position *******/3
{
   if(row==0)
    {
	 lcd_cmd(0x80+column);
...
void lcd_cmd(unsigned char cmd) //*****************************************/4
{
	PORTC&=~0x07;
	PORTC=((cmd&0xF0));			
	latch();
...
void lcd_char(unsigned char single) //*************************************/5
{
	PORTC=0x01;
	PORTC=0x00;
...
void lcd_string(unsigned char str[32]) //**********************************/6
{
	unsigned char k=0;
	PORTC=0x01;
	PORTC=0X00;
...
void lcd_showvalue(unsigned char num) //prints the decimal 3digit value of num *******/7
{
